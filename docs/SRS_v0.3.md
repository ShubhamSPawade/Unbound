...
```
Controller Layer  →  Service Layer  →  Repository Layer  →  Entity Layer  →  PostgreSQL
```
...
College
  └── Fest (optional)
        └── Event (SOLO or TEAM, FREE or PAID)
              └── Registration (Student)

College
  └── Club
        └── Event (standalone — no fest required)
              └── Registration (Student)
```

### 3.5 Core User Flows

**Student Registration Flow (Solo Event):**
```
Student browses events → Selects event → Clicks Register
→ If FREE: Registration confirmed immediately
→ If PAID: Razorpay payment initiated → On success: Registration confirmed
```

**Student Registration Flow (Team Event):**
```
Student browses events → Selects team event → Creates team
→ Adds members (within min/max team size) → Submits registration
→ If FREE: Registration confirmed immediately
→ If PAID: Razorpay payment initiated → On success: Registration confirmed
```
...
```
CLUB_ADMIN logs in → Creates event (DRAFT) → Sets participation type and fee
→ Publishes event → Students can now discover and register
```

**Club Approval Flow:**
```
CLUB_ADMIN registers club → Status: PENDING
→ COLLEGE_ADMIN reviews → Approves or Rejects (with reason)
→ If Approved: Club visible to students
```
...
6. Client sends token in Authorization: Bearer header
7. JwtFilter validates token on every request
